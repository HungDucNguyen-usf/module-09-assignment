...
# Your code here
displayed_columns = sales_df[["Product", "Units", "Total_Sales"]]
...
#check the Date column, find the ones that starts with Feb
...
#check the groups of products, check the total sales, sum all the total sales and find the one with the highest
...
#Group all the regions together, sort by the sum of the total sales descending
...
#Group by the category, then find the mean of each units column per group
...
# Your code here
promo_items = sales_df[sales_df["Promotion"] == "Yes"] #create 2 groups: the promo and nonpromo
nonpromo_items = sales_df[sales_df["Promotion"] == "No"]
promo_comparison = {
    "promo_avg_sales" : promo_items["Total_Sales"].mean(), #avg sale promo
    "no_promo_avg_sales": nonpromo_items["Total_Sales"].mean(), #avg sale non promo
    "promo_total_revenue": promo_items["Total_Sales"].sum(), #total sale promo
    "no_promo_total_revenue": nonpromo_items["Total_Sales"].sum(), #total sale nonpromo
    }
...
# Your code here
missing_counts = sales_df.isnull().sum() #make a True/False Dataframe, then sum to count each of the missing values per columns
...
# Your code here
missing_percentages = sales_df.isnull().mean()*100 #mean for proportion of True, then 100 for percentage
...
# Your code here
top_categories_by_region = (sales_df.groupby(["Region", "Category"])["Total_Sales"].sum().groupby(level=0).idxmax().str[1])
#sum the total sales, then group for each of the regions, then take the top sales category, get str 1 since the codegrade just want the name
...
#group for each category, then find the average unit price using mean
...
# Your code here
total_sales_product = sales_df.groupby('Product')['Total_Sales'].sum() #make a total sales for each product
product_revenue_analysis = pd.DataFrame({ #combines all into a 2 columns table
    'total_revenue': total_sales_product, 
    'percentage': total_sales_product/total_revenue * 100
    })
...
# Your code here
print("\nData Quality Report:")
cols_with_missing = missing_counts[missing_counts > 0].index.tolist() #make a list only for the columns with missing values
print(f"- Missing Values Found: {cols_with_missing}") #name of missing values
print(f"- Total Missing Entries: {int(missing_counts.sum())}") #total missing values

# 7.6 Provide three key business recommendations based on your analysis
print("\nKEY BUSINESS RECOMMENDATIONS:")
# Your code and recommendations here
print("1/ Recommend the business addresses the missing data in the Units column and Unit_Price column")
print("2/ Recommend the business expands more in Latin America, since it generated the least revenue")
print("3/ Recommend the business sells more Smartphones as it is the top performing category in 3 regions ")